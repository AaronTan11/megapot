...
// Reads from MEGAETH_RPC_URL env var, falls back to thirdweb RPC if not set
const MEGAETH_RPC_URL =
	process.env.MEGAETH_RPC_URL || "https://6343.rpc.thirdweb.com";
const EXPECTED_CHAIN_ID = 6343;
...
async function testChainId(): Promise<void> {
	...
	if (chainId === EXPECTED_CHAIN_ID) {
		console.log(
			`   ✅ Chain ID matches MegaETH Testnet (${EXPECTED_CHAIN_ID})`
		);
	} else {
		console.log(
			`   ⚠️ Unexpected chain ID. Expected ${EXPECTED_CHAIN_ID}, got ${chainId}`
		);
	}
}
...
async function testBlockNumber(): Promise<void> {
	...
	console.log(
		`   Block number (dec): ${blockNumber.toLocaleString()}`
	);
...
async function testLatestBlock(): Promise<void> {
	...
	const block = await rpcCall<Block>("eth_getBlockByNumber", [
		"latest",
		false,
	]);
...
async function main(): Promise<void> {
	...
	console.log("\n📝 Notes:");
	console.log(
		"   - WebSocket methods (eth_subscribe) are currently rate-limited"
	);
	console.log("   - See https://docs.megaeth.com/faq for details");
	console.log(
		"   - MegaETH has 10ms mini-blocks for realtime confirmations"
	);
